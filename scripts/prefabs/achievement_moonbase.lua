...
local function IsFixed(inst)
    return inst.components.workable.workleft > TUNING.MOONBASE_DAMAGED_WORK + 1
end
...
local function GetAnimState(inst)
    return (IsFixed(inst) and "full")
        or (inst.components.workable.workleft > TUNING.MOONBASE_DAMAGED_WORK and "med")
        or (inst.components.workable.workleft >= 3 and "medlow")
        or "low"
end
...
local function fn()
    ...
    inst.components.workable:SetMaxWork(TUNING.MOONBASE_COMPLETE_WORK+1)
    inst.components.workable:SetWorkLeft(TUNING.MOONBASE_DAMAGED_WORK+3)
...
return Prefab("achievement_moonbase", fn, assets, prefabs)
