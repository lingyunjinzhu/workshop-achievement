...
local NO_TAGS = { "bramble_resistant", "INLIMBO", "notarget", "noattack", "flight", "invisible", "playerghost" , "bee"}
...
local function MakeFX(name, anim, damage)
    local function fn()
        ...
        inst:AddTag("FX")
        inst:AddTag("thorny")
        if name == "achiv_bramblefx_trap" then
...
return MakeFX("achiv_bramblefx_armor", "idle", "ARMORBRAMBLE_DMG"--[[TUNING.ARMORBRAMBLE_DMG]]),
    MakeFX("achiv_bramblefx_trap", "trap", "TRAP_BRAMBLE_DAMAGE"--[[TUNING.TRAP_BRAMBLE_DAMAGE]])
