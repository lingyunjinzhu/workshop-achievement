...
local function SetSize(inst, sz, mod)
    ...
    if sz == "medium" then
        ...
        if rand <= 0.15 then  
            inst.loot =
            {
                ...
                { prefab = "rock1", chance = TUNING.METEOR_CHANCE_INVITEM_ALWAYS * mod },
    ...
    elseif sz == "large" then
        local rand = math.random()
        if rand <= TUNING.METEOR_CHANCE_BOULDERMOON * mod then   --0.15
            inst.loot =
            {
                {
                    prefab = "rock_moon",
...
return Prefab("shadowmeteor_ai", fn, assets, prefabs)
